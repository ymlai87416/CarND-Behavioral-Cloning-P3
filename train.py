...
############################################################
#                                                          #
#       First pass of the training                         #
#       Only train the network to run on track 1           #
#                                                          #
############################################################
...
############################################################
#                                                          #
#       Second pass of the training                        #
#       Train the network to run on track 1 & 2            #
#                                                          #
############################################################
...
with open("./train_data/track_2_normal/driving_log.csv") as csvfile:
    reader = csv.reader(csvfile)
    for line in reader:
            lines.append(line)
...
for line in lines:
    ...
    for i in range(3):
        source_path = line[i]
        filename = source_path.split('\\')[-1]
        current_path = './train_data/track_2_normal/IMG/' + filename
        image = Image.open(current_path)
        image = np.array(image.convert('YCbCr'))
...
############################################################
#                                                          #
#       Third pass of the training                         #
#       Train the network to run on track 1 & 2 (enhanced) #
#                                                          #
############################################################
